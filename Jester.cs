namespace AdvanceGame
{
    ...
    public class Jester : Piece
    {
        ...
        public override List<Tuple<int, int>> GetValidMoves(Board board, int startX, int startY)
        {
            ...
            int[] deltaXOptions = new int[] { -1, 0, 1 };
            int[] deltaYOptions = new int[] { -1, 0, 1 };

            //for loop to check all possible moves
            for (int i = 0; i < deltaXOptions.Length; i++)
            {
                for (int j = 0; j < deltaYOptions.Length; j++)
                {
                    if (deltaXOptions[i] == 0 && deltaYOptions[j] == 0)
                    {
                        continue;
                    }

                    int endX = startX + deltaXOptions[i];
                    int endY = startY + deltaYOptions[j];
        ...
        public bool IsValidConversion(char[,] board, int startX, int startY, int endX, int endY)
        {
            ...
            if ((deltaX == 1 && deltaY <= 1) || (deltaX == 0 && deltaY == 1))
            {
                ...
                if (IsEnemyPiece(destinationPiece) && char.ToUpper(destinationPiece) != 'G')
                {
                    return true;
                }
            }
        ...
        public override bool IsValidMove(Board board, int startX, int startY, int endX, int endY)
        {
            ...
            if ((deltaX == 1 && deltaY <= 1) || (deltaX == 0 && deltaY == 1))
            {
                ...
                if (destinationPiece == '.' || IsFriendlyPiece(destinationPiece))
                {
                    return true;
                }
            }
