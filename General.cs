...
public class General : Piece
{
    ...
    {
        var tempGeneral = new General(Symbol, Player);
    ...
    public override List<Tuple<int, int>> GetValidMoves(Board board, int startX, int startY)
    {
        ...
        foreach (var deltaX in deltaXOptions)
        foreach (var deltaY in deltaYOptions)
    ...
    public Tuple<int, int>? IdentifyThreat(Board board, int posX, int posY)
    {
        ...
        foreach (var pieceType in pieceTypes)
            ...
            for (var y = 0; y < board.Cols; y++)
            {
                ...
                {
                    var enemyPiece = PieceFactory.CreatePiece(piece, Player == "white" ? "black" : "white");
        ...
        foreach (var pieceType in pieceTypes)
            ...
            for (var y = 0; y < board.Cols; y++)
            {
                ...
                if (IsEnemyPiece(piece) && piece == pieceType)
                {
                    ...
                    if (enemyPiece != null && enemyPiece.IsValidMove(board, x, y, posX, posY)) return true;
                }
            }
