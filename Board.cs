namespace AdvanceGame
{
    ...
    public class Board
    {
        ...
        public static bool WouldBeProtected(Board board, int sentinelX, int sentinelY, int generalX, int generalY, string currentPlayer)
        {
            int[] dx = { 0, -1, 0, 1 };
            int[] dy = { -1, 0, 1, 0 };
        ...
        public bool IsGeneralInDangerAfterMove(int startX, int startY, int endX, int endY, string currentPlayer, bool checkNextTurn = true)
        {
            ...
            bool IsInDanger = general.IsInDanger(tempBoard, endX, endY);
            return IsInDanger;
        }
        ...
        public bool IsGeneralProtectedAfterWallConstruction(int builderX, int builderY, int wallX, int wallY, int generalX, int generalY, string currentPlayer)
        {
            // Store the original value at the wall position
            char original = _board[wallX, wallY];
            ...
            General general = new(currentPlayer == "white" ? 'G' : 'g', currentPlayer);
            bool isInDanger = general.IsInDanger(this, generalX, generalY);
        ...
        {
            General general = new General(currentPlayer == "white" ? 'G' : 'g', currentPlayer);
            Tuple<int, int>? generalPosition = FindGeneralPosition(general);

            if (generalPosition != null && general.IsInDanger(this, generalPosition.Item1, generalPosition.Item2))
            {
                Tuple<int, int>? threat = general.IdentifyThreat(this, generalPosition.Item1, generalPosition.Item2);
                if (threat != null && TryCaptureThreateningPiece(currentPlayer, threat))
                    return;

                Tuple<int, int, int, int>? bestProtectionMove = FindProtectionMove(currentPlayer, generalPosition);
                ...
                Tuple<int, int>? safeMove = general.FindSafestMove(this, generalPosition.Item1, generalPosition.Item2);
                if (safeMove != null)
                {
        ...
        {
            char piece = GetPiece(startX, startY);

            string playerOfPiece = IsCorrectColor(piece, "white") ? "white" : "black";

            if (piece == 'J' || piece == 'j')
            {
                char endPiece = GetPiece(endX, endY);
        ...
        private bool EvaluateAndExecuteMove(int startX, int startY, int endX, int endY, string currentPlayer, ref Tuple<int, int, int, int>? bestCaptureMove, ref Tuple<int, int, int, int>? bestNonCaptureMove)
        {
            bool moveMade = false;
        ...
        private Tuple<int, int, int, int>? FindProtectionMove(string currentPlayer, Tuple<int, int> generalPosition)
        {
            ...
            for (int startX = 0; startX < Rows; startX++)
            {
                ...
                {
                    char piece = GetPiece(startX, startY);
                    Piece? pieceObj = PieceFactory.CreatePiece(piece, currentPlayer);
                        ...
                        (currentPlayer == "black" && piece == 's')) // If the piece is a sentinel
                    {
                        ...
                        for (int i = 0; i < 8; i++)
                        {
                            ...
                            if (pieceObj != null && pieceObj.IsValidMove(this, startX, startY, endX, endY) &&
                                WouldBeProtected(this, endX, endY, generalPosition.Item1, generalPosition.Item2, currentPlayer) &&
            ...
            if (bestProtectionMove != null)
            {
                MovePiece(bestProtectionMove.Item1, bestProtectionMove.Item2, bestProtectionMove.Item3, bestProtectionMove.Item4);
                CurrentBoard = _board;
        ...
        private void MakeBestNonCaptureOrDangerousMove(Tuple<int, int, int, int>? bestNonCaptureMove, Tuple<int, int, int, int>? bestDangerousMove)
        {
                        ...
                        (currentPlayer == "black" && piece == 'j')) // If the piece is a Jester
                    {
                        ...
                        for (int i = 0; i < 8; i++)
                        {
                            ...
                            if (pieceObj is Jester &&
                                ((Jester)pieceObj).IsValidConversion(this._board, startX, startY, endX, endY))
                            {
                                if (threatPosition != null && endX == threatPosition.Item1 && endY == threatPosition.Item2)
                                {
                                    return Tuple.Create(startX, startY, endX, endY);
                                }
                                if (bestConversionMove == null)
                                {
                                    bestConversionMove = Tuple.Create(startX, startY, endX, endY);
                                }
                            }
                        }
                    }
        ...
        private void TryMakeMove(string currentPlayer)
        {
            ...
            for (int startX = 0; startX < Rows && !moveMade; startX++)
            {
                for (int startY = 0; startY < Cols && !moveMade; startY++)
                {
                    Tuple<int, int, int, int>? jesterConversionMove = TryConvertEnemyWithJester(currentPlayer);
                    if (jesterConversionMove != null)
                    {
                        MoveAndUpdateBoard(jesterConversionMove.Item1, jesterConversionMove.Item2, jesterConversionMove.Item3, jesterConversionMove.Item4);
                        return;
                    }
                    char piece = GetPiece(startX, startY);
                    Piece? pieceObj = PieceFactory.CreatePiece(piece, currentPlayer);
                    ...
                    if (piece != '.' && pieceObj?.Player == currentPlayer && IsCorrectColor(piece, currentPlayer))
                    {
                        moveMade = TryMovesForPiece(startX, startY, currentPlayer, pieceObj, ref bestCaptureMove, ref bestNonCaptureMove, ref bestDangerousMove);
                    }
                }
            }
            ...
            if (!moveMade)
            {
                MakeBestNonCaptureOrDangerousMove(bestNonCaptureMove, bestDangerousMove);
            }
        }
        ...
        private bool TryMovesForPiece(int startX, int startY, string currentPlayer, Piece pieceObj, ref Tuple<int, int, int, int>? bestCaptureMove, ref Tuple<int, int, int, int>? bestNonCaptureMove, ref Tuple<int, int, int, int>? bestDangerousMove)
        {
            bool moveMade = false;

            for (int endX = 0; endX < Rows && !moveMade; endX++)
            {
                for (int endY = 0; endY < Cols && !moveMade; endY++)
                {
                    if (pieceObj.IsValidMove(this, startX, startY, endX, endY) &&
                        !IsGeneralInDangerAfterMove(startX, startY, endX, endY, currentPlayer, false))
                    {
                        moveMade = EvaluateAndExecuteMove(startX, startY, endX, endY, currentPlayer, ref bestCaptureMove, ref bestNonCaptureMove);
                    }
